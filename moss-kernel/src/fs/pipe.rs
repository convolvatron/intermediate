...
impl PipeReader {
    async fn do_read(&self, read_fut: impl Future<Output = Result<usize, KernelError>>) -> Result<usize, KernelError> {
...
impl FileOps for PipeReader {
    async fn read(&mut self, _ctx: &mut FileCtx, u_buf: UA, count: usize) -> Result<usize, KernelError> {
        if count == 0 {
            return Ok(0);
        }
    ...
    async fn write(&mut self, _ctx: &mut FileCtx, _buf: UA, _count: usize) -> Result<usize, KernelError> {
...
impl PipeWriter {
    async fn do_write(&self, write_fut: impl Future<Output = Result<usize, KernelError>>) -> Result<usize, KernelError> {
...
impl FileOps for PipeWriter {
    async fn read(&mut self, _ctx: &mut FileCtx, _buf: UA, _count: usize) -> Result<usize, KernelError> {
        Err(KernelError::BadFd)
    }

    async fn write(&mut self, _ctx: &mut FileCtx, u_buf: UA, count: usize) -> Result<usize, KernelError> {
        if count == 0 {
            return Ok(0);
        }
