...
impl FileOps for RegFile {
    ...
    async fn write(&mut self, ctx: &mut FileCtx, mut buf: UA, mut count: usize) -> Result<usize, KernelError> {
        let mut pg = ClaimedPage::alloc_zeroed()?;
        let kbuf = pg.as_slice_mut();
        let mut total_bytes_read = 0;
    ...
    async fn seek(&mut self, ctx: &mut FileCtx, pos: SeekFrom) -> Result<u64, KernelError> {
