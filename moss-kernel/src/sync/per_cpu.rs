...
macro_rules! per_cpu {
    ($vis:vis static $name:ident: $type:ty = $initializer:expr;) => {
        $vis static $name: crate::sync::per_cpu::PerCpu<$type, $crate::arch::ArchImpl> =
            crate::sync::per_cpu::PerCpu::new($initializer);
        ...
        static [<$name _PERCPU_INITIALIZER>]: &'static (
                     dyn crate::sync::per_cpu::PerCpuInitializer + Sync
                 ) = &$name;
        }
    };
}
