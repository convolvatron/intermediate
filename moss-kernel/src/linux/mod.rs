...
pub trait Filesystem: Send + Sync {
    ...
    async fn root_inode(&self) -> Result<Arc<dyn Inode>, KernelError>;
    ...
    async fn sync(&self) -> Result<(), KernelError> {
        Ok(())
    }
}
...
pub trait DirStream: Send + Sync {
    ...
    async fn next_entry(&mut self) -> Result<Option<Dirent>, KernelError>;
}
