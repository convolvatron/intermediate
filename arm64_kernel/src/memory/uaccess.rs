...
pub async fn copy_to_user<T: UserCopyable>(dst: TUA<T>, obj: T) -> Result<(), KernelError> {
    unsafe {
...
pub async fn copy_from_user<T: UserCopyable>(src: TUA<T>) -> Result<T, KernelError> {
...
pub async fn copy_objs_to_user<T: UserCopyable>(src: &[T], mut dst: TUA<T>) -> Result<(), KernelError> {
    for obj in src {
...
pub async fn copy_from_user_slice(src: UA, dst: &mut [u8]) -> Result<(), KernelError> {
    unsafe { ArchImpl::copy_from_user(src, dst.as_mut_ptr() as *mut _ as *mut _, dst.len()).await }
}

pub async fn copy_to_user_slice(src: &[u8], dst: UA) -> Result<(), KernelError> {
