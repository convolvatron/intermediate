...
use crate::{
    ...
    arch::ArchImpl,
    memory::{
        PAGE_SIZE, address::UA, kbuf::KBufCore,
        page::ClaimedPage,
...
impl KPipe {
    ...
    pub async fn copy_from_user(&self, src: UA, count: usize) -> Result<usize, KernelError> {
    ...
    pub async fn copy_to_user(&self, dst: UA, count: usize) -> Result<usize, KernelError> {
        let mut temp_buf = [0u8; USER_COPY_CHUNK_SIZE];
        let chunk_buf = &mut temp_buf[..min(count, USER_COPY_CHUNK_SIZE)];
