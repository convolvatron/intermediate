...
fn load_kernel_aligned(pathname:String) -> Result<&'static mut [u8], std::io::Error> {
    ...
    f.read(ptr)?;
    let file = elf::ElfBytes::<elf::endian::LittleEndian>::minimal_parse(ptr).expect("elf parse kernel");
/*
...
impl VM {
    ...
    fn guest_to_host(&self, source:u64) -> Result<u64, std::io::Error> {
        match self.regions.lower_bound(std::ops::Bound::Included(&source)).peek_prev() {
            Some(x) => {
                Ok(x.1.0 + (source - x.0))
            },
            None => {
                Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "missing region"))
            }
        }
...
fn vm_create() {
   ...
   // should use start address and .. you know
    vm.map(kernel[0x10000..].as_ptr() as *const u8,
           EL1_USER_PAYLOAD_ADDRESS,           
           (kernel.len() - 0x10000) as usize,
           MemPerm::ExecReadWrite).unwrap();
