...
use crate::{dir::OpenFileDirIter, file::FileCtx, syscalls::iov::IoVec};
...
pub trait FileOps: Send + Sync {
    ...
    async fn read(&mut self, ctx: &mut FileCtx, buf: UA, count: usize) -> Result<usize, KernelError>;

    /// Writes data from `buf` to the current file position.
    /// The file's cursor is advanced by the number of bytes written.
    async fn write(&mut self, ctx: &mut FileCtx, buf: UA, count: usize) -> Result<usize, KernelError>;

    async fn readv(&mut self, ctx: &mut FileCtx, iovecs: &[IoVec]) -> Result<usize, KernelError> {
        process_iovec!(iovecs, |addr, count| self.read(ctx, addr, count)).await
    }

    async fn readdir<'a>(&'a mut self, _ctx: &'a mut FileCtx) -> Result<OpenFileDirIter<'a>, KernelError> {
        Err(FsError::NotADirectory.into())
    }

    async fn writev(&mut self, ctx: &mut FileCtx, iovecs: &[IoVec]) -> Result<usize, KernelError> {
    ...
    /// block.
    fn poll_read_ready(&self) -> Pin<Box<dyn Future<Output = Result<(), KernelError>> + 'static + Send>> {
        Box::pin(async { Err(KernelError::NotSupported) })
    }

    /// Puts the current task to sleep until a call to `write()` would no longer
    /// block.
    fn poll_write_ready(&self) -> Pin<Box<dyn Future<Output = Result<(), KernelError>> + 'static + Send>> {
        Box::pin(async { Err(KernelError::NotSupported) })
    }

    /// Moves the file's cursor to a new position.
    /// Returns the new position from the start of the file.
    async fn seek(&mut self, _ctx: &mut FileCtx, _pos: SeekFrom) -> Result<u64, KernelError> {
        Err(KernelError::NotSupported)
    }

    /// Performs a device-specific control operation.
    async fn ioctl(&mut self, _ctx: &mut FileCtx, _request: usize, _argp: usize) -> Result<usize, KernelError> {
